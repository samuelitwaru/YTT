   ...
   public class employeehourslistdetailexport : GXProcedure
   {
      ...
      protected void S141( )
      {
         ...
         while ( AV38GXV1 <= AV23SDTEmployeeHours.Count )
         {
            ...
            AV37ColumnsWithSec = 0;
            GXt_char1 = "";
            new GeneXus.Programs.wwpbaseobjects.wwp_export_securetext(context ).execute(  AV17SDTEmployeeHoursItem.gxTpr_Employeename, out  GXt_char1) ;
            AV11ExcelDocument.get_Cells(AV14CellRow, AV15FirstColumn+0, 1, 1).Text = GXt_char1;
            if ( AV36IsAuthorizedSDTEmployeeHours )
            {
               GXt_char1 = "";
               new GeneXus.Programs.wwpbaseobjects.wwp_export_securetext(context ).execute(  AV17SDTEmployeeHoursItem.gxTpr_Formattedworkhours, out  GXt_char1) ;
            ...
            {
               GXt_char1 = "";
               new GeneXus.Programs.wwpbaseobjects.wwp_export_securetext(context ).execute(  AV17SDTEmployeeHoursItem.gxTpr_Formattedleavehours, out  GXt_char1) ;
      ...
      protected void S191( )
      {
         ...
         {
            AV21GridState.FromXml(new GeneXus.Programs.wwpbaseobjects.loadgridstate(context).executeUdp(  "EmployeeHoursListDetailGridState"), null, "", "");
         }
         else
         {
