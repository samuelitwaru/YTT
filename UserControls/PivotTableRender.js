function PivotTable($) {
	...
	var template = '<style>	/* Freeze the last 3 columns */	td:nth-last-child(1), th:nth-last-child(1),	td:nth-last-child(2), th:nth-last-child(2),	td:nth-last-child(3), th:nth-last-child(3) {		position: sticky;		right: 0;		background-color: white; /* Adjust as needed */		z-index: 2;		min-width:60px;	}	td:nth-last-child(2), th:nth-last-child(2) {		right: 60px; /* Adjust width according to column width */	}		td:nth-last-child(3), th:nth-last-child(3) {		right: 120px; /* Adjust width according to column width */	}</style><div id=\"print\" style=\"padding-top:1px;overflow:scroll; scrollbar-width:none; border-right: 1px solid #dddddd; border-left: 1px solid #dddddd\">	<table class=\"my-sticky-column-table gx-tab-spacing-fix-2 GridWithPaginationBar GridWithBorderColor WorkWith table-responsive\" style=\'border: 1px solid #dddddd; border-collapse: collapse;\'>		<thead  style=\"border-collapse: collapse; position:sticky; top:0; z-index:10\">			<tr>				<th class=\'text-center\' style=\"border: 1px solid #dddddd; border-collapse: collapse; position:sticky;left:0; background:white;\">Projects per employee:</th>				{{#SDTProjects}}				<th style=\"border: 1px solid #dddddd; border-collapse: collapse;padding: 5px; background:#f5f5f5;\" scope=\"col\" class=\"text-center\"><a id=\'Project-{{Id}}\' style=\'cursor:pointer\' class=\'project\' >{{ProjectName}}</a></th>				{{/SDTProjects}}				<th style=\"border: 1px solid #dddddd; border-collapse: collapse; padding:5px; background:#f5f5f5;\" class=\"work text-center\">Total Work Hours</th>				{{#ShowLeaveTotal}}<th style=\"border: 1px solid #dddddd; border-collapse: collapse; padding:5px; background:#f5f5f5;\" class=\"leave text-center\">Total Leave Hours</th><th style=\"border: 1px solid #dddddd; border-collapse: collapse; padding:5px; background:#f5f5f5;\" class=\"leave text-center\">Total</th>{{/ShowLeaveTotal}}			</tr>		</thead>						<tbody style=\'max-height:100px\'>			{{#SDTEmployeeProjectHoursCollection}}			<tr class=\"GridWithPaginationBar GridNoBorder WorkWithOdd text-center\">					<td style=\"border: 1px solid #dddddd; border-collapse: collapse;font-weight: bold;background:#f5f5f5; position:sticky; left:0; z-index:1\" class=\'text-center\'><a href=\'\'  data-event=\"Click\" >{{EmployeeName}}</a></td>								{{#SDTProjects}}				<td id=\'{{EmployeeId}}-{{Id}}\' style=\'border: 1px solid #dddddd; border-collapse: collapse;\' class=\"text-center\"></td>				{{/SDTProjects}}								<td class=\"work text-center\" style=\'border: 1px solid #dddddd; border-collapse: collapse;font-weight: bold;background:#f5f5f5;\'>					<span class=\"tag\" style=\'padding: 0.5rem; {{#IsWorkTimeOptimal}} background:#00a95c; color:white; border-radius:5px;{{/IsWorkTimeOptimal}}\'>{{TotalFormattedTime}}</span>				</td>				{{#ShowLeaveTotal}}<td class=\"leave text-center\" style=\'border: 1px solid #dddddd;font-weight: bold; border-collapse: collapse;background:#f5f5f5;\'>{{TotalFormattedLeave}}</td>{{/ShowLeaveTotal}}				{{#ShowLeaveTotal}}<td class=\'leave\' style=\'border: 1px solid #dddddd;font-weight: bold; border-collapse: collapse;background:#f5f5f5;\'>{{FormattedTotal}}</td>{{/ShowLeaveTotal}}							</tr>			{{/SDTEmployeeProjectHoursCollection}}		</tbody>				<tfoot>			<tr class=\"GridWithPaginationBar GridNoBorder WorkWithOdd\">				<td style=\"border: 1px solid #dddddd; border-collapse: collapse;font-weight: bold;background:#fff; position:sticky; left:0; z-index:1\">Total</td>				{{#SDTProjects}}				<td class=\"project-total text-center\" id=\'{{Id}}\' style=\'border: 1px solid #dddddd; border-collapse: collapse;\' scope=\"col\">0</td>				{{/SDTProjects}}				<td id=\'totalWorkHours\' class=\"work text-center\" style=\'border: 1px solid #dddddd; border-collapse: collapse;bold;background:#f5f5f5;\'>{{TotalFormattedWorkTime}}</td>				{{#ShowLeaveTotal}}<td class=\"leave text-center\" style=\'border: 1px solid #dddddd; border-collapse: collapse;\'></td>{{/ShowLeaveTotal}}				{{#ShowLeaveTotal}}<td class=\"leave text-center\" style=\'border: 1px solid #dddddd; border-collapse: collapse;\'>{{TotalFormattedTime}}</td>{{/ShowLeaveTotal}}							</tr>		<tfoot>			</table></div><script type=\"text/javascript\">    $(document).ready(function() {			$(window).on(\'resize\', function() {			// Your code here			var newHeight = $(window).height();			console.log(\"New height: \" + newHeight);			$(\'#print\').css(\"height\", newHeight-100)			// Perform actions based on the new height		});  });  </script>';
	var partials = {  }; 
