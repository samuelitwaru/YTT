   ...
   public class atestproc : GXWebProcedure
   {
      ...
      protected override void ExecutePrivate( )
      {
         ...
         if ( ! String.IsNullOrEmpty(StringUtil.RTrim( AV8fileName)) && ! String.IsNullOrEmpty(StringUtil.RTrim( AV9name)) )
         {
            ...
            {
               AV11httpResponse.AppendHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
            }
            if ( ! context.isAjaxRequest( ) )
            {
               AV11httpResponse.AppendHeader("X-Frame-Options", "deny");
            }
            if ( ! context.isAjaxRequest( ) )
            {
               AV11httpResponse.AppendHeader("Type-Options", "nosniff");
            }
            if ( ! context.isAjaxRequest( ) )
            {
               AV11httpResponse.AppendHeader("Content-Disposition", "attachment;filename="+AV9name);
            }
            AV11httpResponse.AddFile(AV8fileName);
         }
         else
         {
            AV11httpResponse.AddString("Not found");
         }
