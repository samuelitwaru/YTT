...
namespace GeneXus.Programs {
   ...
   public class leaverequestpendingexport__default : DataStoreHelperBase, IDataStoreHelper
   {
                                             ...
                                             string A132LeaveRequestStatus )
      {
         ...
         {
            AddWhere(sWhereString, "(( LOWER(T3.EmployeeName) like '%' || LOWER(:lV60Leaverequestpendingds_1_filterfulltext)) or ( LOWER(T2.LeaveTypeName) like '%' || LOWER(:lV60Leaverequestpendingds_1_filterfulltext)) or ( LOWER(T1.LeaveRequestHalfDay) like '%' || LOWER(:lV60Leaverequestpendingds_1_filterfulltext)) or ( SUBSTR(TO_CHAR(T1.LeaveRequestDuration,'90.9'), 2) like '%' || :lV60Leaverequestpendingds_1_filterfulltext) or ( SUBSTR(TO_CHAR(T3.EmployeeBalance,'90.9'), 2) like '%' || :lV60Leaverequestpendingds_1_filterfulltext))");
         }
         else
         {
            GXv_int4[0] = 1;
