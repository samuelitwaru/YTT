   ...
   public class reportdetail4 : GXDataArea
   {
      ...
      protected void RF4Z2( )
      {
         ...
         if ( ! context.WillRedirect( ) && ( context.nUserReturn != 1 ) )
         {
            ...
            {
               A106EmployeeId = H004Z2_A106EmployeeId[0];
      ...
      protected void define_styles( )
      {
         ...
         {
            context.AddJavascriptSource(StringUtil.RTrim( ((string)Form.Jscriptsrc.Item(idxLst))), "?202462821154381", true, true);
            idxLst = (int)(idxLst+1);
         }
         if ( ! outputEnabled )
         {
      ...
      protected void include_jscripts( )
      {
         if ( nGXWrapped != 1 )
         {
            ...
            context.AddJavascriptSource("reportdetail4.js", "?202462821154382", false, true);
            context.AddJavascriptSource("DVelop/Bootstrap/Shared/DVelopBootstrap.js", "", false, true);
            context.AddJavascriptSource("DVelop/Shared/WorkWithPlusCommon.js", "", false, true);
            context.AddJavascriptSource("DVelop/DVPaginationBar/DVPaginationBarRender.js", "", false, true);
            context.AddJavascriptSource("UserControls/PaginationBarRender.js", "", false, true);
      ...
      public override void initialize( )
      {
         ...
         H004Z3_AGRID1_nRecordCount = new long[1] ;
         ...
         pr_default = new DataStoreProvider(context, new GeneXus.Programs.reportdetail4__default(),
            new Object[][] {
                new Object[] {
               H004Z2_A106EmployeeId, H004Z2_A148EmployeeName, H004Z2_A118WorkHourLogId
               }
               , new Object[] {
   ...
   public class reportdetail4__default : DataStoreHelperBase, IDataStoreHelper
   {
    ...
    private void cursorDefinitions( )
    {
       if ( def == null )
       {
          ...
          def= new CursorDef[] {
              new CursorDef("H004Z2", "SELECT DISTINCT NULL AS EmployeeId, EmployeeName, NULL AS WorkHourLogId FROM ( SELECT T1.EmployeeId, T2.EmployeeName, T1.WorkHourLogId FROM (WorkHourLog T1 INNER JOIN Employee T2 ON T2.EmployeeId = T1.EmployeeId) ORDER BY T1.WorkHourLogId) DistinctT ORDER BY WorkHourLogId  OFFSET :GXPagingFrom2 LIMIT CASE WHEN :GXPagingTo2 > 0 THEN :GXPagingTo2 ELSE 1e9 END",false, GxErrorMask.GX_NOMASK | GxErrorMask.GX_MASKLOOPLOCK, false, this,prmH004Z2,11, GxCacheFrequency.OFF ,true,false )
             ,new CursorDef("H004Z3", "SELECT COUNT(*) FROM ( SELECT T2.EmployeeName FROM (WorkHourLog T1 INNER JOIN Employee T2 ON T2.EmployeeId = T1.EmployeeId) GROUP BY T2.EmployeeName) GroupByT ",false, GxErrorMask.GX_NOMASK | GxErrorMask.GX_MASKLOOPLOCK, false, this,prmH004Z3,1, GxCacheFrequency.OFF ,true,false )
          };
       }
    }
