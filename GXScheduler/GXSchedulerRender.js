function GXScheduler() {
	...
	this.SetDefaultStart = function (data) {
		this.SetDefaultStart = this.GetNormalizedDate(data, true);
	}

	this.GetDefaultStart = function () {
		return this.SetDefaultStart;
	}

	this.SetDefaultEnd = function (data) {
		this.SetDefaultEnd= this.GetNormalizedDate(data, true);		
	}

	this.GetDefaultEnd = function () {
		...
		scheduler.attachEvent("onBeforeLightbox", function (event_id){			
			if( this.getState().mode=="month" && typeof gxSchedulerUC.GetDefaultStart().valueOf()=="number"){
				...
				if(evSugStart.getHours()!=0 && evSugStart.getUTCFullYear()!=2001){
					...
					scheduler.setEventStartDate(event_id,new Date(anio,mes,dia,hora,min));
				}
			}
			if(this.getState().mode=="month" && typeof gxSchedulerUC.GetDefaultEnd().valueOf()=="number"){
				...
				if(evSugEnd.getHours()!=0 && evSugEnd.getUTCFullYear()!=2001){
					...
					scheduler.setEventEndDate(event_id,new Date(anio,mes,dia,hora,min));
			}				
			}							
			return true;
		});
		function MergeEventSuggestedClicked(evSug,evClick){
			var anio = 0; var mes = 0; var dia =0 ;var hora=0;var min=0;			
				if(evSug.getHours()==0){		//edito solo la fecha										
					anio =	evSug.getFullYear();
					mes  =	evSug.getMonth();
					dia  =	evSug.getDate();
					hora =	evClick.getHours();
					min	 = 	evClick.getMinutes();
					return  [anio,mes,dia,hora,min];										
				}else{							//edito solo la hora
					anio = 	evClick.getFullYear();
					mes  = 	evClick.getMonth();
					dia  = 	evClick.getDate();
					hora =	evSug.getHours();
					min  =	evSug.getMinutes();
					return [anio,mes,dia,hora,min];
				}																			
		}
	...
	this.ConvertDayGxSdtToDhxSdt = function (gxDay_object) {
		...
		day.date = new Date(d.getFullYear(), d.getMonth(), d.getDate(), 0, 0, 0, 0);
